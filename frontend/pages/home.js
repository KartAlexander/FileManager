...
const Home = () => {
  ...
  const handleDownload = async (fileId) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        // Извлекаем имя файла с поддержкой различных форматов
        let filename = 'file';
        
        if (contentDisposition) {
          // Обрабатываем разные форматы заголовка Content-Disposition
          ...
          // Используем имя из списка файлов, если оно там есть
        ...
        // Преобразуем ответ в blob и скачиваем
        ...
        showMessage(`Файл ${filename} успешно скачан`, 'success');
      } else {
  ...
  // Функция для отображения информации о файле (в будущем может быть расширена)
  ...
  //     `Дата загрузки: ${formatDate(file.uploaded_at)}`,
  //     `Статус: ${file.is_encrypted ? 'Зашифрован' : 'Без шифрования'}`
  //   ].join('\n');
  ...
  const showFileInfo = (file) => {
    setModalContent(
      <div className='flex flex-col justify-center items-center text-center space-y-2'>
        ...
        <p><strong>Дата загрузки:</strong> {formatDate(file.uploaded_at)}</p>
        <p><strong>Статус:</strong> {file.is_encrypted ? "Зашифрован" : "Без шифрования"}</p>
      </div>
    );
    setModalOpen(true);
  };
  ...
  const handleDelete = async (fileId) => {
    ...
    try {
      const response = await fetch(`${backendUrl}/api/files/delete/${fileId}/`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${token}`,
        },
                  ...
                  <div className="p-6">
                    ...
                    <div className="mb-4">
                      ...
                      <p className="mt-2 text-xs text-gray-500 dark:text-gray-400 flex justify-center transition text-center">
                        {encrypt 
                          ? 'Файл будет зашифрован с использованием алгоритма RSA' 
                    ...
                    <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                      ...
                      <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700/50">
                        {filteredFiles.map((file) => (
                          <tr key={file.id} className="hover:bg-gray-50/70 dark:hover:bg-gray-700/50 transition">                            <td className="px-6 py-4 whitespace-nowrap">
                              <div className="flex items-center">
